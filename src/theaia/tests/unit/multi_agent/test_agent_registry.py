...
class TestAgentRegistryStaleHeartbeats:
    def test_check_stale_heartbeats_no_stale(self, registry):
        agent = AgentMetadata(agent_type="ActiveAgent")
        registry.register(agent)
        stale = registry.check_stale_heartbeats()
        assert isinstance(stale, list)
        assert len(stale) == 0

    def test_check_stale_heartbeats_with_stale(self, registry):
        agent = AgentMetadata(agent_type="StaleAgent")
        agent_id = registry.register(agent)
        retrieved = registry.get(agent_id)
        if retrieved and hasattr(retrieved, 'last_heartbeat'):
            retrieved.last_heartbeat = datetime.now() - timedelta(seconds=120)
        stale = registry.check_stale_heartbeats()
        assert isinstance(stale, list)
        assert len(stale) >= 0
    ...
    def test_mark_stale_as_unavailable(self, registry):
        ...
        if retrieved and hasattr(retrieved, 'last_heartbeat'):
            retrieved.last_heartbeat = datetime.now() - timedelta(seconds=120)
        marked = registry.mark_stale_as_unavailable()
...
class TestAgentRegistryEdgeCases:
    ...
    def test_unregister_nonexistent_agent(self, registry):
        result = registry.unregister("nonexistent-id-12345")
        assert result in [False, None]

    def test_update_status_nonexistent(self, registry):
        result = registry.update_status("nonexistent-id", AgentStatus.MAINTENANCE)
        assert result in [False, None]

    def test_heartbeat_nonexistent(self, registry):
        registry.update_heartbeat("nonexistent-id")
        assert True

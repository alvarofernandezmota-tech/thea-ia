...
class ConversationManager:
    """
    [translate:Manager principal que orquesta la conversación y delega a managers especializados.]
    """
    ...
    def process_input(self, message: str, candidate_intents: List[str]) -> Tuple[str, str, Dict[str, Any]]:
        try:
            ...
            # [translate:Si la conversación anterior ya había terminado, se resetea ANTES de procesar el nuevo mensaje.]
            if self.state == GlobalState.COMPLETED.value:
                self.fsm.reset()

            # [translate:Ahora, se procesa la entrada con el estado ya limpio (si era necesario).]
    ...
    def _handle_agent_delegated_state(self, message: str) -> Tuple[str, str, Dict[str, Any]]:
        ...
        if active_agent_intent == 'agenda':
            ...
            # [translate:Si el especialista terminó, solo marcamos el estado global como 'completed'.]
            # [translate:El reseteo se hará en el siguiente turno.]
    ...
    # ... [translate:El resto de los métodos se mantienen exactamente igual]
